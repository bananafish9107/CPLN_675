...
Here is a color palette of hexidecimal color codes which we can feed to our symbology options.

```{r color_ramp, echo=TRUE}
ramp8 <- c("#050234","#2C0078","#7F00BF","#F600FF","#FF0DBE","#FF569F","#FF9BA8","#FFE8E4")
```

OK - so let's try that first map again, but we can add a customized color ramp using `scale_fill_gradientn`, as well as some labels and titles. Let's also apply a more pleasing visual theme to our ggplot using `theme_void` - one of many ggplot theme options.

...
NEXT STEPS - RESAMPLE TO LARGER CELLS, REDO WORKFLOW TO CREATE 2011 and 2019 SETS SEPARATELY SO VARIABLES HAVE SAME NAME FOR RUNNING IN THE MODEL.

Take the functions and stash them in a separate .R file - make a little appendix describing the function arguments

Make the workflow correspond to a present-day time period that is the same as the assignment - 2009-2019 perhaps. DONE

Set up an introduction that describes the workflow in bullet points, lays out the alternative workflow in ArcGIS.

Get rid of the plotTheme and mapTheme DONE

Remove spatial CV - too advanced for 675

Change to workflow so that it's bringing in lc data from both years, identifying change etc., This is the cleaning routine students will be doing. DONE

Downsample the rasters a bit more, the whole thing is running pretty slowly.

Provide additional workflow overview - broad outline of what the steps are....
...
The `get_nlcd` function from the package `FedData` can be used to fetch NLCD data and then crop it to a `template` that is a `sf` shape that you specify. For us, we will use the `houstonMSA`.

These data aren't available for every year, so we will use 2011 and 2019 data as our time interval - there is an NLCD set for each year. 
...
Let's look at some info about these rasters - they are 30m grid cells.

```{r}
lc_2019
```

We want to resample these to be much larger (because at 30m, trust me, they will crush our laptops when we try to process stuff).

Let's resample them to be 20x coarser resolution. You will make your own resampling deliberation when you do this analysis yourself for your homework. The resolution depends a lot on your study area size.
...
Reclassify our land cover databases to consist of 1 and 0 observations (e.g. 1 is the developed classes 13-24, 0 is everything else). 
...
Let's use Mapview to take a look at the change
...
Create fishnet at same resolution as raster..

note the crs and resolution that are called here
...
houstonMSA_fishnet <- 
  st_make_grid(houstonMSA %>%
               ...
               square = TRUE) %>%
   .[houstonMSA %>%
            st_transform(crs(development_change))] %>%            # clips the grid to the extent
  st_sf() %>%
  mutate(uniqueID = rownames(.))
```

### Aggregate LC Change to fishnet
...
ggplot() +
  geom_sf(data=houstonMSA %>% st_transform(crs(fishnet))) +
...
### Aggregate 2011, 2019 LC to fishnet

MF - THIS GETS SCREWED UP IF YOU DON'T RESAMPLE USING MODAL AS THE FUNCTION

The table below shows the approach taken to recoded existing land cover codes into the categories used in our analysis. In the code block below new rasters are generated for each lanc cover type and `names` are applied. These rasters will then all be aggregated to our fishnet, giving us land cover info for each cell.

Naming ensures that when the raster is integrated with the fishnet, the column reflects the appropriate raster.
...
ggplot() +
  ...
  theme_void(), ncol=2)
```
Aggregate Census Data to the fishnet using aerially weighted interpolation.
...
### DL Highways, calculate dist to fishnet centroids
...
First highway vectors (`primary_roads) are downloaded from the Tigris; projected and subset to the study area using `st_intersection`. Below, new development is mapped with the highway overlay.
...
We can create a new fishnet variable called `distance_highways` by turning our `houstonMSA_fishnet` into centroid points and calculating the distance from the cell centroid to the nearest highway segment. This is done in a `mutate` command that utilizes the `st_distance` function to run this distance calculation for each observation.

If you wanted to model the effect of a new highway in your model to project future development, you could import a new, modified highway network, and create a new variable representing highway distance.


```{r}
highwayPoints_fishnet <- houstonMSA_fishnet %>%
  st_centroid() %>%
  mutate(distance_highways = as.numeric(st_distance(., st_union(houstonHighways) %>% 
                                                      st_transform(st_crs(houstonMSA_fishnet))))) %>%
  as.data.frame() %>% 
...
ggplot() +
  geom_sf(data=houstonMSA %>% st_transform(st_crs(highwayPoints_fishnet))) +
...
There is a lot of good text here from the original to paste in
...
fishnet$lagDevelopment_2011 <-
    nn_function(xyC(fishnet),
                xyC(filter(lcRasters_2011,developed_2011==1)),
                2)

fishnet$lagDevelopment_2019 <-
    nn_function(xyC(fishnet),
                xyC(filter(lcRasters_2019,developed_2019==1)),
                2)
...
### Put final data set together

Let's take stock of what we have.

Shouldn't this be joined all by unique ID? Isn't that what we are using to keep track of stuff?

```{r}


dat_2011 <- 
  cbind( fishnet, highwayPoints_fishnet, fishnetPopulation, lcRasters_2011, countyFishnet) %>%
  as.data.frame() %>%
  dplyr::select(uniqueID, development_change, lagDevelopment_2011, distance_highways, pop_2011,  
...
dat_2019 <- 
  cbind( fishnet, highwayPoints_fishnet, fishnetPopulation, lcRasters_2019, countyFishnet) %>%
  as.data.frame() %>%
  dplyr::select(uniqueID, development_change, lagDevelopment_2019, distance_highways, pop_2019,  
...
```{r, warning = FALSE, message = FALSE}
dat_2011 %>%
  dplyr::select(development_change, developed:otherUndeveloped) %>%
...
### Train/Test models and validate.
...
Print the summary object for your model of choice
...
### Predicting for the whole set, analyzing error
...
Make 2027 forecast, chose your appropriate threshold - here I choose 0.17
...
Assess impacts - how much land converts? sensitive land is lost per county? what are the expected population projections per county? what is your planning prescription given the conversion forecast?
